    ...
    @if(session('throttle_error_message') && session('limit_reached'))
        <div class="limit-popup" style="
            ...
            <a href="{{ route('login') }}" style="
                ...
                onmouseout="this.style.backgroundColor='#c62828'">{{ __('login_now') }}</a>
            <button onclick="this.closest('.limit-popup').remove()" style="
                background: none; border: none; color: #b71c1c; cursor: pointer;
                font-size: 0.9rem; margin-top: 10px; display: block; width: 100%;">Dismiss</button>
        </div>
    @endif
    ...
    @if(session('favorite_limit_error'))
        <div class="favorite-limit-popup" style="
            ...
            <button onclick="this.closest('.favorite-limit-popup').remove()" style="
                ...
                font-weight: bold; border-radius: 6px; transition: background-color 0.2s;"
                onmouseover="this.style.backgroundColor='#d32f2f'" onmouseout="this.style.backgroundColor='#c62828'">Close</button>
        </div>
    @endif
    ...
    @if(session('favorite_status'))
        <div class="favorite-success-popup" style="
            ...
            <button onclick="this.closest('.favorite-success-popup').remove()" style="
                ...
                padding: 10px 20px; margin-top: 20px; cursor: pointer;
                font-weight: bold; border-radius: 6px; transition: background-color 0.2s;"
                onmouseover="this.style.backgroundColor='#388e3c'" onmouseout="this.style.backgroundColor='#2e7d32'">Dismiss</button>
        </div>
    @endif

    <div x-data="{ sidebarOpen: true }" class="flex min-h-screen">

        <aside class="bg-white shadow-md transition-all duration-300 flex flex-col" :class="sidebarOpen ? 'w-64' : 'w-0 overflow-hidden'">
            <div class="px-6 py-4 text-lg font-bold border-b flex items-center gap-3">
                <img src="{{ asset('storage/ReadSpaceLogo.png') }}"
                alt="ReadSpace Logo"
            ...
            <nav class="p-4 space-y-2 flex-1">
                ...
                <div class="px-4 py-2 text-xs uppercase text-gray-500 font-semibold tracking-wide">{{ __('features') }}</div>

                <div x-data="{ open: false }">
                    <button @click="open = !open" class="w-full flex justify-between items-center px-4 py-2 rounded hover:bg-slate-100">
                ...
                <a href="{{ route('categories.index') }}" class="block px-4 py-2 rounded hover:bg-slate-100">{{ __('categories') }}</a>

                @auth
                    <a href="{{ route('favorites.index') }}" class="block px-4 py-2 rounded hover:bg-slate-100">{{ __('favorite_books') }}</a>
                @else
                    <span class="block px-4 py-2 rounded text-gray-400 cursor-not-allowed" title="{{ __('login_required_favorites') }}">{{ __('favorite_books') }}</span>
                @endauth
                ...
                @auth
                    @if(auth()->user()->role === 'admin')
                        <hr class="my-2">
                        <div class="px-4 py-2 text-xs uppercase text-gray-500 font-semibold tracking-wide">{{ __('admin_panel') }}</div>
                        <a href="{{ route('admin.users.index') }}" class="block px-4 py-2 rounded hover:bg-slate-100">{{ __('user_management') }}</a>
                        <a href="{{ route('admin.activity.index') }}" class="block px-4 py-2 rounded hover:bg-slate-100"> {{ __('activity_log') }}</a>
                    @endif
                @endauth
            ...
            <header class="bg-white shadow px-6 py-4 flex justify-between items-center relative">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex items-center gap-2">
                        <a href="{{ route('locale.switch', ['locale' => 'en']) }}" class="px-3 py-1 rounded border text-sm hover:bg-slate-100 {{ $loc==='en'?'bg-slate-200 font-semibold':'' }}">EN</a>
                        <a href="{{ route('locale.switch', ['locale' => 'id']) }}" class="px-3 py-1 rounded border text-sm hover:bg-slate-100 {{ $loc==='id'?'bg-slate-200 font-semibold':'' }}">ID</a>
                    </div>

                    <div class="relative" x-data="{ open: false }">
                        @auth
                            <button @click="open = !open" class="w-10 h-10 rounded-full bg-slate-800 text-white flex items-center justify-center">{{ strtoupper(substr(auth()->user()->name,0,1)) }}</button>
                            <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 mt-2 w-40 bg-white border shadow rounded">
                                <a href="{{ route('profile.edit') }}" class="block px-4 py-2 text-sm hover:bg-slate-100">{{ __('profile') }}</a>
                                <form method="POST" action="{{ route('logout') }}">@csrf<button class="w-full text-left px-4 py-2 text-sm hover:bg-slate-100">{{ __('sign_out') }}</button></form>
                            </div>
                        @else
                            <button @click="open = !open" class="w-10 h-10 rounded-full bg-gray-300 text-gray-600 flex items-center justify-center">G</button>
                            <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 mt-2 w-40 bg-white border shadow rounded">
                                <a href="{{ route('login') }}" class="block px-4 py-2 text-sm hover:bg-slate-100">{{ __('sign_in') }}</a>
                            </div>
                        @endauth
                    </div>
                </div>
