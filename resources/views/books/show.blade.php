    ...
    <div class="flex gap-6">
      ...
      <div class="flex-1 flex flex-col justify-between">
        <div>
          ...
          <p class="text-gray-500">{{ __('by_author_lower', ['author' => $book->author]) }}</p>
          ...
          @else
            ...
            <div class="flex-1 flex flex-col justify-between">
                <div>
                    ...
                    <div class="mt-4">
                        <p class="text-sm text-gray-600">{{ $book->publisher }} • {{ $book->year }} • {{ $book->genre }}
                        </p>
                    </div>
                </div>
                    ...
                    @auth
                        ...
                        @else
                            <form action="{{ route('books.favorite', $book) }}" method="POST" class="flex items-center">
                                @csrf
                                <button
                                    class="flex items-center justify-center px-4 h-10 border rounded hover:bg-gray-100 text-base font-normal">
                                    {{ auth()->user()->favorites->contains($book->id) ? 'Unfavorite' : 'Add to Favorite' }}
                                </button>
                            </form>
                            @if ($book->pdf_path)
                                {{-- <a href="{{ route('books.pdf', $book) }}"
                                    class="flex items-center justify-center px-4 h-10 rounded
              bg-blue-600 hover:bg-blue-700 text-white text-base font-normal">
                                    Download PDF
                                </a> --}}
                                <a href="{{ route('books.pdf', $book) }}"
